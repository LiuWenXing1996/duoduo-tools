...
> = {
  ...
  Rankine: {
    ...
    fromKelvin: (val) => (val * 9) / 5,
    display: (val) => `${val} °R`,
  },
  Delisle: {
    label: "德利尔 (Delisle)",
    toKelvin: (val) => 373.15 - (val * 2) / 3,
    fromKelvin: (val) => ((373.15 - val) * 3) / 2,
    display: (val) => `${val} °D`,
  },
  Newton: {
    label: "牛顿 (Newton)",
    toKelvin: (val) => (val * 100) / 33 + 273.15,
    fromKelvin: (val) => ((val - 273.15) * 33) / 100,
    display: (val) => `${val} °N`,
  },
  ...
  Reaumur: {
    ...
    fromKelvin: (val) => ((val - 273.15) * 4) / 5,
    display: (val) => `${val} °Ré`,
  },
  Romer: {
    label: "罗氏度 (Rømer)",
    toKelvin: (val) => ((val - 7.5) * 40) / 21 + 273.15,
    fromKelvin: (val) => ((val - 273.15) * 21) / 40 + 7.5,
    display: (val) => `${val} °Rø`,
  },
};
