...
> = {
  Kelvin: {
    ...
    toKelvin: (val) => val,
    formKelvin: (val) => val,
    display: (val) => `${val.toFixed(2)} K`,
  },
  Celsius: {
    label: "摄氏度 (Celsius)",
    toKelvin: (val) => val + 273.15,
    formKelvin: (val) => val - 273.15,
    display: (val) => `${val.toFixed(2)} °C`,
  },
  Fahrenheit: {
